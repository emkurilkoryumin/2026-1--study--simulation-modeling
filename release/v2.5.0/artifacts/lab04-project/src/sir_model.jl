...
days_infected::Int
 # количество дней с момента заражения
status::Symbol 
# :S, :I, :R
end
...
Ns = [1000, 1000, 1000], 
# численность по трём городам
migration_rates = nothing, 
# матрица миграции
β_und = [0.5, 0.5, 0.5], 
# передача невыявленными
β_det = [0.05, 0.05, 0.05], 
# передача выявленными
infection_period = 14,
detection_time = 7,
death_rate = 0.02,
...
model.β_und[agent.pos]
 # невыявленный — высокая заразность
else
model.β_det[agent.pos]
 # выявленный — низкая заразность
end
